...
from models.policy import StraightThroughPolicy2D, LOUPEPolicy2D
from fastmri.models.varnet import NormUnet

from utils.utils import create_dir_and_save_image
...
class AdVarNet2D(nn.Module):
    ...
    def __init__(
        ...
        if self.loupe_mask :
            ...
            self.policy = StraightThroughPolicy2D(
                crop_size = self.crop_size,
                budget = self.budget,
    ...
    def forward(
        ...
        if self.loupe_mask : 
            mask, masked_kspace, prob_mask = self.loupe.do_acquisition(
                kspace, mask
            )
        else :
            mask, masked_kspace, prob_mask = self.loupe.do_acquisition(
        ...
        #extra_outputs["prob_mask"].append(prob_mask[0,0,:,:,0].detach().cpu())
        extra_outputs["prob_mask"].append(prob_mask[0,0,:,:,0]) # for prob_reg_loss
        ...
        for cascade in self.cascades:
            ...
            # Store current reconstruction
            current_recon = fastmri.complex_abs(
                self.sens_reduce(kspace_pred, sens_maps)
            ).squeeze(1)
