...
class VarNet2DModule(MriModule):
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if vmap_target_path is not None :
            vmap = torch.Tensor(np.load(vmap_target_path))
            create_dir_and_save_image(vmap / vmap.max(), self.default_root_dir + "/masks", f'vmap.png')
    ...
    def training_step(self, batch, batch_idx):
        ...
        create_dir_and_save_image(extra_outputs["acquire_mask"][0].float(), self.default_root_dir + "/masks", f'subsampling_mask.png')
