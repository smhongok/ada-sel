...
from fastmri.models import AdVarNet
...
class AdVarNetModule(MriModule):
    """
    AdVarNet training module.
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.varnet = AdVarNet(
            deq=self.deq,
            learn_acquisition=self.learn_acquisition,
    ...
    def forward(self, kspace, masked_kspace, mask, num_low_frequencies):
        return self.varnet(kspace, masked_kspace, mask, num_low_frequencies)

    def training_step(self, batch, batch_idx):
        output = self(batch.kspace, batch.masked_kspace, batch.mask, batch.num_low_frequencies)
    ...
    def validation_step(self, batch, batch_idx):
        output = self.forward(
            batch.kspace, batch.masked_kspace, batch.mask, batch.num_low_frequencies
        )
        target, output = transforms.center_crop_to_smallest(batch.target, output)
    ...
    def add_model_specific_args(parent_parser):  # pragma: no-cover
        ...
        parser.add_argument(
            ...
            default=False,
            type=str2bool,
            help="Deep equilibritum model",
        )        
        parser.add_argument(
            "--learn_acquisition",
            default=True,
            type=str2bool,
            help="Learn acquisition",
        )       
        parser.add_argument(
            "--num_cascades",
            default=12,
