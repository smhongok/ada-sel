...
def t(array): return torch.Tensor(np.expand_dims(array,0).astype(np.float32) / 255)
...
def gray(t): return (
        np.clip((t[0] if len(t.shape) == 4 else t).detach().cpu().numpy(), 0, 1) * 255).astype(
    np.uint8).squeeze()
...
def impad(img, top=0, bottom=0, left=0, right=0, color=255):
    return np.pad(img, [(top, bottom), (left, right)], 'reflect')
...
class HRLRDatasetV(Dataset):
    ...
    def __getitem__(self, index):
        ...
        lr = imread(lr_path)
        #print("getitem.lr.shape",lr.shape) # (16,16,3)
        pad_factor = 2
        # Pad image to be % 2
...
if __name__=="__main__":
    ...
    parser.add_argument('--lrtest_path', default='/data/fastmri/nyu_fastmri_knee_compressed_LRHR/multicoil_val/LR/', type=str) # lr test
    parser.add_argument('--conf_path', default='./confs/8X_mr.yml', type=str)
    parser.add_argument('--output_path', default="./experiments/8x/", type=str) # output path
    parser.add_argument('--n_imgs', type=int, default=7135)
    ...
    if os.path.isdir(test_root[:-1]+ '_vmap') is False: os.mkdir(test_root[:-1]+ '_vmap')
    ...
    for i, data in enumerate(progress_bar):
        ...
        if i in check:
            with torch.no_grad():
                ...
                srs_k = np.empty((n_sample,h* scale,w* scale),dtype=np.complex64)
                for ii in range(n_sample):
                    ...
                    # imwrite(path_out_sr, sr)
                    # #print(sr.shape) 
                vmap = np.var(srs_k, axis=0)
                path_out_vmap = os.path.join(test_root[:-1]+'_vmap/', f'{i:06d}.npy')
                with open(path_out_vmap, 'wb') as f:
                    np.save(f,vmap)
