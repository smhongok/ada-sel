...
class StraightThroughPolicy(nn.Module):
    ...
    def forward(self, kspace_pred: torch.Tensor, mask: torch.Tensor):
        ...
        quadratic = True
        if quadratic:
            probs_to_norm[:,0] = probs_to_norm[:,0] * 5/6 + probs_to_norm[:,1]/6
            probs_to_norm[:,-1] = probs_to_norm[:,-1] * 5/6 + probs_to_norm[:,-2]/6
            probs_to_norm[:,1:-1] = probs_to_norm[:,1:-1]*2/3 + probs_to_norm[:,0:-2]/6 + probs_to_norm[:,2:]/6
...
class LineConvSampler(nn.Module):
    ...
    def forward(self, image, mask):
        ...
        if self.use_softplus:
            ...
            # print("maskdtype",mask.dtype)
            prob_mask = out / torch.max(
                (~mask.reshape(out.shape[0], out.shape[1])) * out, dim=1
            )[0].reshape(-1, 1)
        else:
            prob_mask = torch.sigmoid(self.slope * out)
        # Mask out already sampled rows
        prob_mask = prob_mask * (
            ~mask.reshape(prob_mask.shape[0], prob_mask.shape[1])
        )
