...
class AdVarNetSample(NamedTuple):
    """
    A sample of masked k-space for variational network reconstruction.
...
class MiniCoilTransform:
    ...
    def __call__(self, kspace, mask, target, attrs, fname, slice_num):
        ...
        #print(kspace_complex.shape, kspace_complex.dtype)
        new_shape = (num_compressed_coils,) + kspace_complex.shape[1:]
        kspace_complex= np.reshape(kspace_complex, (kspace_complex.shape[0], -1))
        left_vec, _, _ = np.linalg.svd(kspace_complex, compute_uv=True, full_matrices=False)
        kspace_complex = np.reshape(
            np.array(np.matrix(left_vec[:, :num_compressed_coils]).H @ kspace_complex),
            new_shape,
        )
        kspace = to_tensor(kspace_complex)
